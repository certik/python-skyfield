! ═══════════════════════════════════════════════════════════════════════
! moon_ekf.f90 — 2-body Earth-Moon EKF with N-body propagation
! ═══════════════════════════════════════════════════════════════════════
!
! State vector: x = [e, i, Ω, ω, M₀, μ, a]  (7 elements, all constant)
!   e    = eccentricity of Moon orbit
!   i    = inclination of Moon orbit to equator (~28.5 deg)
!   Ω    = longitude of ascending node
!   ω    = argument of periapsis
!   M₀   = mean anomaly at reference epoch
!   μ    = GM_earth + GM_moon (total gravitational parameter)
!   a    = semi-major axis of Moon orbit (km)
!
! Fixed parameters:
!   R_E  = Earth radius (6378.137 km)
!
! Key physics: The Moon is close enough (~386,000 km) that diurnal
!   parallax from the observer's position on Earth's surface (~6378 km)
!   is ~57 arcmin — directly constraining the distance a.
!   This breaks the μ-a degeneracy that exists for the distant Sun.
!
...
! Observations: Moon alt/az from observations.dat
