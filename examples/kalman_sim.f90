...
!  kalman_sim.f90
!
!  3-body (Sun, Earth, Moon) UKF estimation from backyard observations.
!
!  Reads observations.dat (noisy alt/az), propagates a Newtonian 3-body
!  simulation with a scaled Unscented Kalman Filter, and estimates:
!    - GM values (masses) of Sun, Earth, Moon
!    - Orbital distances (Earth-Sun, Earth-Moon)
!
!  State vector (21):
!    x(1:3)=Sun pos, x(4:6)=Sun vel,
!    x(7:9)=Earth pos, x(10:12)=Earth vel,
!    x(13:15)=Moon pos, x(16:18)=Moon vel,
!    x(19)=GM_sun, x(20)=GM_earth, x(21)=GM_moon
!
!  Compile:  flang kalman_sim.f90 -o kalman_sim
!  Run:      ./kalman_sim
! ═══════════════════════════════════════════════════════════════════════
...
module constants_mod
  ...
  real(dp), parameter :: AU_KM = 149597870.700_dp
  real(dp), parameter :: ASEC2RAD = 4.848136811095359935899141d-6

  ! WGS84
  real(dp), parameter :: WGS84_RADIUS = 6378137.0_dp  ! m
  real(dp), parameter :: WGS84_INVF   = 298.257223563_dp
end module constants_mod


! ─────────────────────────────────────────────────────────────────────
!  Module 2: SPK reader (from newton_sim.f90)
! ─────────────────────────────────────────────────────────────────────
...
! ─────────────────────────────────────────────────────────────────────
!  Module 3: 3-body Newtonian dynamics
! ─────────────────────────────────────────────────────────────────────
module nbody3_mod
  use constants_mod
  implicit none
...
contains
  ...
  subroutine read_observations(filename, obs, n_obs, jd_max)
    ...
    real(dp), intent(in) :: jd_max   ! only read up to this JD
    ...
    do
      ...
      if (n_obs > MAX_OBS) then
        print *, 'WARNING: max observations exceeded, truncating'
        n_obs = MAX_OBS
        exit
      end if
...
module ukf_mod
  use constants_mod
  use nbody3_mod, only: NX, propagate_state
